...
class FolderShareConnect {
   ...
   */
  const VERSION = '0.6.2 (March 2019)';
   ...
   */
  private function getSerializationUrlOption() {
...
EOS;
  ...
  protected function verifyServer() {
    ...
    //
    // Set up OPTIONS request
    // ----------------------
    // Use an OPTIONS request, which returns in the header a list of the
    // HTTP verbs supported (e.g. GET, POST, PATCH, DELETE). The request
    // should not return a body and we need the header. To parse the
    // header, we nominate a header callback function.
    //
    ...
    $url = $this->hostName . self::URL_GET . '/0';
    ...
    //
    // The header verification callback function nominated above will be
    // called when the HTTP header is returned, and THAT function sets
    // the $httpVerbs array with the results of the query.
    $this->printVerbose("Verifying server connection with OPTIONS query");

    // Clear the HTTP verbs supported. This will be filled in by the header
    // callback during execution of this request.
    $this->httpVerbs = [];
    ...
    // Decode JSON content. The content just contains an error message when
    // an HTTP code indicates a problem.
    ...
    if ($httpCode !== 200) {
      ...
      if (isset($content['message']) === TRUE) {
        ...
        if ($httpCode === 404) {
          $message = <<<'EOS'
The host declined the connection.
...
EOS;
    ...
    $this->printVerbose("  Supported OPTIONS: " . implode(', ', $this->httpVerbs));

    //
    // Verify minimal support
    // ----------------------
    // Header verification has filled in $httpVerbs. Make sure GET is
    // listed. While DELETE, POST, and PATCH can be disabled, if GET is not
    // available, then there is little point to this server connection.
    if (in_array('GET', $this->httpVerbs) === FALSE) {
      $message = <<<'EOS'
The host does not support web services.
This can occur if the host does not have the FolderShare module enabled, or if web services for the module have not been enabled. Please contact the host's administrator.
EOS;
      $this->printVerbose("  Server failed with no GET support from OPTIONS query");
      $this->printVerbose("  $message");
      throw new \RuntimeException($message);
    }
    ...
    // Get configuration
    // -----------------
    // Query the site's configuration. Part of the response lists the
    // authentication providers and serialization formats supported by the
    // server.
    //
    ...
    //
    // Since we haven't done this query yet, we don't know what serialization
    // formats are supported on the site yet. We need to try them in sequence
    // and hope to find one that works.
    try {
      // Try the JSON serialization format first. This is the most common
      // and the one we prefer.
      $this->serializationFormat = self::SERIALIZATION_JSON;
      $config = $this->getServerConfiguration();
    }
    catch (\Exception $e) {
      // Try the JSONHAL serialization format next. This is less common,
      // but we can work with it too since it is basically just JSON.
      try {
        $this->serializationFormat = self::SERIALIZATION_JSONHAL;
        $config = $this->getServerConfiguration();
      }
      catch (\Exception $e) {
        // Query failed.
        //
        // The error message returned by the Drupal REST module is very
        // obscure - it says that the {foldershare} parameter on the route
        // could not be unserialized, which is literally true since we
        // couldn't find an agreeable serialization format. But that is
        // hardly the kind of message to show the user.
        $this->serializationFormat = self::SERIALIZATION_NONE;
        $this->serverVerified = FALSE;
        $this->clearCsrfToken();
        $message = <<<'EOS'
...
EOS;
    ...
    // Reconcile GET, POST, DELETE, and PATCH. While the above query has
    // returned a list of HTTP verbs supported by the site, it is possible
    // for the list to be more inclusive than the site really supports.
    // Make sure there is configuration information for each HTTP verb,
    // and disable that verb if not.
    $reconciledHttpVerbs = [];
    foreach ($this->httpVerbs as $verb) {
      if (isset($config[$verb]) === TRUE &&
          empty($config[$verb]['authentication-providers']) === FALSE &&
          empty($config[$verb]['serializer-formats']) === FALSE) {
        $reconciledHttpVerbs[] = $verb;
      }
    }

    $this->httpVerbs = $reconciledHttpVerbs;

    //
    // Re-verify minimal support
    // -------------------------
    // $httpVerbs has been updated based on authentication and serialization
    // support. Make sure GET is still listed. While DELETE, POST, and PATCH
    // can be disabled, if GET is not available, then there is little point
...
This can occur if the host does not have the FolderShare module enabled, or if web services for the module have not been enabled. Please contact the host's administrator.
EOS;
      $this->printVerbose("  Server failed with no GET support");
      $this->printVerbose("  $message");
      $this->serverVerified = FALSE;
      $this->clearCsrfToken();
      throw new \RuntimeException($message);
...
EOS;
  ...
  private function logoutForCookie() {
    ...
    // - The logout token.
    $url = $this->hostName .
      self::URL_USER_LOGOUT .
      $this->getSerializationUrlOption();
