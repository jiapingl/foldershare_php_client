               Web services client for the Drupal FolderShare module
...
The client application is composed of three PHP files in the 'src' directory:
  ...
  - Main.php is the main application code.
  - FolderShareConnect.php is the client communications API.
...
However, executing the application in this way requires that the other
source files be in the same directory. This is not ideal.
...
The application's help describes how to use the application and send
...
For example, the PHP code to get version numbers from the server looks
like this:
...
You can read through 'Main.php' to see how these classes are used.
